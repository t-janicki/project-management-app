...
function UserNavbarHeader(props)
{
    ...
    return (
        ...
        >
            <Typography className="username text-16 whitespace-no-wrap" color="inherit">{user.name} {user.lastName} </Typography>
            <Typography className="email text-13 mt-8 opacity-50 whitespace-no-wrap" color="inherit">{user.email}</Typography>
            <Avatar
                className={clsx(classes.avatar, "avatar")}
                alt="user photo"
                src={user.avatarUrl && user.avatarUrl !== '' ? user.avatarUrl : "assets/images/avatars/profile.jpg"}
            />
        </AppBar>
    );
}
