...
mock.onGet('/api/products').reply(() => {
    return [200, productsDB.products]
});

mock.onPost('/api/products/add-editProduct').reply((request) => {
...
mock.onPost('/api/products/update-editProduct').reply((request) => {
    const data = JSON.parse(request.data);

    productsDB.products = productsDB.products.map((product) => {
...
mock.onPost('/api/products/remove-editProduct').reply( (request) => {
    const data = JSON.parse(request.data);

    productsDB.products = productsDB.products.filter((product) => data.productId !== product.id);

    return [200, productsDB.products]
});

mock.onPost('/api/products/remove-products').reply( (request) => {
    const data = JSON.parse(request.data);

    productsDB.products = productsDB.products.filter((product) => !data.productIds.includes(product.id));

    return [200, productsDB.products]
});

mock.onGet('/api/product').reply((request) => {
    const {productId} = request.params;
    const response = _.find(productsDB.products, {'id' : productId});
    return [200, response];
});
